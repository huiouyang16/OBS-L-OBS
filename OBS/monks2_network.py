"""
    This code is used to define the monks2 network,
    train/load the trained the monks2 network,
    calculate/load the hessian inv matrix of trained the monks2 network,
    pruned/save the monks2 network,
...
# define the monk2 model
# structure: 17 - Linear - sigmoid - 2 - Linear - sigmoid - 1
class MONK2Network(nn.Module):

    def __init__(self):
        super(MONK2Network, self).__init__()
        self.fc1 = nn.Linear(17, 2)
...
if __name__ == '__main__':
    ...
    # get a monk1 model
    monk2_network = MONK2Network()

    # get the train set and test set
    train_set = MONKDataset('/../dataset/monks-problems/monks-2.train')
    train_loader = data.DataLoader(train_set, shuffle=True)
    print('train set already')
    test_set = MONKDataset('/../dataset/monks-problems/monks-2.test')
    ...
    optimizer = torch.optim.SGD(monk2_network.parameters(), lr=0.1, momentum=0.9)
    if not os.path.exists('monk2_model/trained/monk2_network.pkl') or re_train:
        ...
        torch.save(monk2_network.state_dict(), 'monk2_model/trained/monk2_network.pkl')
    else:
        monk2_network.load_state_dict(torch.load('monk2_model/trained//monk2_network.pkl'))
    ...
    # calculate or load the hessian inv matrix of the monk2 model
    if not os.path.exists('monk2_model/hessian_inv/monk2_hessian_inv.npy') or re_hessian_inv:
        H_inv = alpha * np.identity(39)
        for i, data in enumerate(train_loader, 0):
            if i % 10 == 9:
                print(i + 1)
            ...
            outputs = monk2_network(inputs)
            loss = criterion(outputs, labels)
            grad_x = torch.autograd.grad(loss, monk2_network.parameters(), create_graph=True)

            X_k = []
            X_k += grad_x[0].view([34]).tolist()
            X_k += grad_x[1].view([2]).tolist()
            X_k += grad_x[2].view([2]).tolist()
            X_k += grad_x[3].view([1]).tolist()
            X_k = np.array(X_k).reshape([39, 1])
            ...
            np.save('monk2_model/hessian_inv/monk2_hessian_inv', H_inv, allow_pickle=True, fix_imports=True)
    else:
        H_inv = np.load('monk2_model/hessian_inv/monk2_hessian_inv.npy')
    ...
    if display_process:
        ...
        count = 0
        Lq = []
        for i in monk2_network.parameters():
